  ...
  for (const year of [1969, 1970, 1971, 1972]) {
    ...
    for (const view of views)
      vehicle.views[view].studio = {
        root: `/designer/studio/chevrolet-${model.toLowerCase()}-${sourceYear}-color-v${model === 'C10' ? sourceYear === 1971 ? 6 : 4 : sourceYear === 1970 ? 5 : 4}/${view}`,
        paintScene: true,
        width: 768,
        height: 512,
        viewport: [0, 0, 768, 512],
...
for (const vehicle of vehicles.filter((v) => ['K10', 'K5', 'F-150'].includes(v.model))) {
  ...
  for (const view of views) {
    ...
    for (const wheel of kmcWheelOptions) {
      ...
      if (vehicle.model === 'K5') vehicle.views[view].studio!.openTopWheelScenes![wheel.id] = {
        stock: `${root}/top-off/${view}.png`,
      };
    }
  }
}
